...
class WechatPay {
	...
	const URL_TRANSFERS = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
    const URL_BIZPAYURL = 'weixin://wxpay/bizpayurl?';
	/**
	 * 错误信息
	 */
	public $error = null;
	...
	private function get_nonce_string($length = 32) {
	...
	private function sign($data) {
		...
		$stringSignTemp = $string1 . "key=" . $this->_config["mch_key"];
     ...
     * 扫码支付(模式一）获取支付二维码
     */
    public function getCodeUrlOne($product_id)
    {
        $data = array(
            'appid'         => $this->_config["appid"],
            'mch_id'        => $this->_config["mch_id"],
            'time_stamp'    => time(),
            'nonce_str'     => $this->get_nonce_string(8),
            'product_id'    => $product_id,
        );
        $sign = $this->sign($data);
        return self::URL_BIZPAYURL . "appid={$data['appid']}&mch_id={$data['mch_id']}&nonce_str={$data['nonce_str']}&product_id={$product_id}&time_stamp={$data['time_stamp']}&sign={$sign}";
    }
    
    /**
     * 获取扫码支付（模式一）的响应数据
     * @param type $return_code
     * @param type $return_msg
     * @param type $prepay_id
     * @param type $result_code
     * @param type $err_code_des
     * @return type
     */
    public function getOneResponseData($return_code, $return_msg, $prepay_id, $result_code, $err_code_des = '')
    {
        $data = array();
        $data['appid']          = $this->_config["appid"];
        $data['mch_id']         = $this->_config["mch_id"];
        ...
        $data['nonce_str']      = $this->get_nonce_string(8);
	 ...
	 * @return array
	 */
	public function refund($out_trade_no,$out_refund_no,$total_fee,$refund_fee,$op_user_id, $refund_account = 'REFUND_SOURCE_UNSETTLED_FUNDS'){
		$data = array();
		$data["appid"] = $this->_config["appid"];
		$data["mch_id"] = $this->_config["mch_id"];
		$data["nonce_str"] = $this->get_nonce_string();
     ...
     * 微信转账给用户
     * 
     * @param type $partner_trade_no 商户订单号
     * @param type $openid
     * @param type $amount
     * @param type $desc
     * @param type $spbill_create_ip
     * @param type $check_name
     * @param type $re_user_name
     * @return type
     */
	public function transfer($partner_trade_no, $openid, $amount, $desc, $spbill_create_ip, $check_name = 'FORCE_CHECK', $re_user_name = NULL){
		$data = array();
