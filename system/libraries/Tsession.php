...
class CI_Tsession {
    ...
    public function __construct() {
        ...
        $this->redis = new CI_Tredis(true,0,0,1);
    ...
    public function destroy($sessionName='') {
        $this->set_cookie(self::$sessionId,'',time()-100,'/',$_SERVER['HTTP_HOST']);
    ...
    public function delay() {
        if($this->is_open() == true) {
            ...
            $this->set_cookie(self::$sessionId,self::$sessionName,$time,'/',$_SERVER['HTTP_HOST']);
    ...
    private function open() {
        if($this->is_open() == false) {
            $time = time() + self::$expired;
            $this->set_cookie(self::$sessionId,self::$sessionName,$time,'/',$_SERVER['HTTP_HOST']);
            $this->redis->hash_set_array(self::$folder.self::$sessionName,array(0=>0)); //设置初始值
            $this->redis->time_out(self::$folder.self::$sessionName,$time); //设置过期时间
        }
    }
    /** 判断是否开启session
    ...
    */
    public function get_array() {
        ...
        $tmp = $this->redis->hash_get_array(self::$folder.self::$sessionName);
    ...
    public function set_array($arr) {
        ...
        $this->redis->hash_set_array(self::$folder.self::$sessionName,$arr);
    }
    /* 返回单个值 失败 返回 false
    */
    public function get($key) {
        if($this->is_open() == false){return false;}
        return $this->redis->hash_get(self::$folder.self::$sessionName,$key);
    }
    /** 设置单个值 失败 返回 false 成功 返回数字
     */
    public function set($key,$value) {
        if($this->is_open() == false) return false;
        return $this->redis->hash_get(self::$folder.self::$sessionName,$key,$value);
    }
